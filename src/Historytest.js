  ...
  const fetchCompletedOrders = async () => {
    try {
      ...
      const completedOrders = data.filter((order) => {
        ...
        const isComplete = tags.includes("Complete");
        ...
        return isComplete && (isPOS || isOnline);
      });
          ...
          <thead>
            ...
            {currentOrders.length > 0 ? (
                ...
                <tr key={order.id}>
                    ...
                    <td>{order.source_name === "pos" ? "POS" : "Online Store"}</td> {/* ✅ Channel column */}
                    <td>
                    {order.user_id && staffMap[order.user_id]
                        ? staffMap[order.user_id]
                        : order.processed_by || "Unknown"}
                    </td>
