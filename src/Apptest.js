  ...
  const fetchOrders = async () => {
    try {
      ...
      const filteredOrders = data.filter((order) => {
        ...
        !order.tags?.includes("Complete") &&
        new Date(order.created_at) >= new Date("2025-03-31T15:42:00+08:00");

        const shouldInclude = (isPOS || isOnline || isDraftOrder) && isPaid && isNotComplete && afterCutoff;
