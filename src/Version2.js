...
function App() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      data.forEach((order) => {
        const tagList = order.tags ? order.tags.split(",").map(t => t.trim()) : [];
        const isFromCorrectLocation = order.location_id === 59489419455;
        const isPaid = order.financial_status === "paid";
        const isNotComplete = !tagList.includes("Complete");
        const orderTime = new Date(order.created_at);
        const startDate = new Date("2025-03-31T08:00:00+08:00");
  
        const isPOS = order.source_name === "pos";
        const isOnline = order.source_name === "web" || order.source_name === "online_store";
        const isUnfulfilled = order.fulfillment_status !== "fulfilled";
        const isfulfilled = order.fulfillment_status !== "unfulfilled";
  
        const shouldInclude =
          (
            (isPOS && isFromCorrectLocation) ||
            (isOnline && isUnfulfilled || isfulfilled)
          ) && isPaid && isNotComplete && orderTime >= startDate;
      ...
      const currentOrderIds = filteredOrders.map((order) => order.id);
      const newOrderIds = currentOrderIds.filter((id) => !alertedOrderIds.includes(id));
          ...
          <table className="order-table">
            ...
            <tbody>
              {orders.length > 0 ? (
                ...
                <tr><td colSpan="8" className="no-orders">No orders available</td></tr>
              )}
            </tbody>
          </table>
